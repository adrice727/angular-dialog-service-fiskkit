...
angular.module('dialogs.services',['ui.bootstrap.modal','dialogs.controllers'])
	...
	.provider('dialogs',[function(){
        ...
        /**
         * Custom for fiskkit. Accepts the name of the state, parses it and provides proper params to 
         * the create function.
         *
         * @param	state 				string		the name of the current state
         * @param	loggedIn 			boolean 	is the current user loggen in
         * @param	intialVisit 	boolean		has the user visited this page before
         */
				custom : function(state, loggedIn, initialVisit) {

					var state = state.split('.').join('-');
					var url = 'fiskkit-dialogs/dialog-partials/' + state + '/' + state + '-dialog.html';

					
					var controller = '';
					angular.forEach(state.split('-'), function(state) {
					  controller += state[0].toUpperCase() + state.slice(1);
					});

					controller += 'DialogCtrl';
					this.create(url, controller, {});
